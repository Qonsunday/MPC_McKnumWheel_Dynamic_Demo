...
A = [
    ...
    0 0 0 0  1  0  0         dt        0;
    0 0 0 0  0  1  0         0         dt;
    0 0 0 -dt/tau 0  0  1-dt/tau 0   0;
    0 0 0 0  -dt/tau 0    0 1-dt/tau 0;
    0 0 0 0  0  -dt/tau 0    0         1-dt/tau
];
...
mpc_obj.Weights.ManipulatedVariables = [0.2 0.2 0.2 0.2];%操纵值 就是输入值
...
% 适配黑色背景的文本/刻度/网格颜色
set(gca, 'XColor', 'white', 'YColor', 'white');  % 坐标轴刻度/标签白色
set(gca, 'GridColor', [0.3, 0.3, 0.3]);          % 网格线深灰色（不刺眼）
xlabel('X 位置 (m)', 'FontSize', 11, 'Color', 'white');
ylabel('Y 位置 (m)', 'FontSize', 11, 'Color', 'white');
title('麦克纳姆轮8字轨迹跟踪（MPC）', 'FontSize', 12, 'Color', 'white');

% 绘制轨迹（仅加粗预测轨迹线宽，其他不变）
h_ref = plot(ref_cache(1,:), ref_cache(2,:), 'y--', 'LineWidth', 2.5, 'DisplayName', '参考轨迹');
h_real = plot(state_hist(1,1), state_hist(2,1), 'c-', 'LineWidth', 2, 'DisplayName', '实际轨迹');
% 预测轨迹：线宽从2.5加粗到4，品红色点线更突出
h_pred = plot(nan, nan, 'm.-', 'LineWidth', 4, 'MarkerSize', 10, 'DisplayName', '预测轨迹');
...
% -------------------------- 修复部分：文本背景 + 文本 --------------------------
% 1. 画深灰色背景矩形（兼容旧MATLAB版本）
rect_x = x_min + 0.2;        % 矩形x起点
rect_y = y_max - 0.35;       % 矩形y起点（比文本低0.05，避免遮挡）
rect_width = 2.5;            % 矩形宽度（足够容纳文本）
rect_height = 0.3;           % 矩形高度（足够容纳文本）
h_rect = rectangle('Position', [rect_x, rect_y, rect_width, rect_height], ...
                   'FaceColor', [0.2, 0.2, 0.2], ...  % 深灰色填充
                   'EdgeColor', 'none');              % 隐藏边框

% 2. 叠加白色文本（仅用text函数支持的参数）
h_info = text(rect_x + 0.1, y_max - 0.3, sprintf('步数: 0/%d  时间: %.1fs  误差: %.4fm', steps, 0, pos_errors(1)), ...
             'Color', 'white', 'FontSize', 10);
% ------------------------------------------------------------------------------

% 图例设置（格式正确，轨迹颜色/样式与名称匹配）
legend('Location', 'northwest', 'FontSize', 10, 'Color', 'black', 'TextColor', 'white');
...
set(gcf, 'Color', 'black');
set(gca, 'Color', 'black', 'XColor', 'white', 'YColor', 'white', 'GridColor', [0.3, 0.3, 0.3]);
plot(time(1:steps), w_hist', 'LineWidth', 1.5);
title('车轮转速变化', 'FontSize', 12, 'Color', 'white');
xlabel('时间 (s)', 'FontSize', 11, 'Color', 'white');
ylabel('转速 (rad/s)', 'FontSize', 11, 'Color', 'white');
ylim([-umax-2, umax+2]);
yline(umax, 'r--', '最大转速', 'FontSize', 10, 'Color', 'red');
yline(-umax, 'r--', '最小转速', 'FontSize', 10, 'Color', 'red');
legend('车轮1','车轮2','车轮3','车轮4', 'Location', 'best', 'FontSize', 10, 'Color', 'black', 'TextColor', 'white');
grid on;
...
set(gcf, 'Color', 'black');
set(gca, 'Color', 'black', 'XColor', 'white', 'YColor', 'white', 'GridColor', [0.3, 0.3, 0.3]);
plot(time, pos_errors, 'LineWidth', 1.5, 'Color', [1, 0.5, 0]);  % 橙色误差线（黑背景显眼）
title('位置跟踪误差', 'FontSize', 12, 'Color', 'white');
xlabel('时间 (s)', 'FontSize', 11, 'Color', 'white');
ylabel('误差 (m)', 'FontSize', 11, 'Color', 'white');
