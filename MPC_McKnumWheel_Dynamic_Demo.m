...
% 轨迹显示
h_ref = plot(ref_hist(1,1), ref_hist(2,1), 'r--', 'LineWidth', 2.5, 'DisplayName', '参考轨迹');
...
% 【核心修改】固定Y轴范围为±2，X轴范围根据轨迹自动调整并保持比例
y_min = -2;    % 固定Y轴最小值
y_max = 2;     % 固定Y轴最大值
...
for k = 1:steps
    ...
    %u_opt：当前步的最优控制量（4 维向量，对应 4 个车轮的目标转速，后续会用这个值控制车轮）
    %info：控制过程的辅助信息（info.Xopt 提取了预测轨迹）
    ...
    set(h_ref, 'XData', ref_hist(1,1:k), 'YData', ref_hist(2,1:k));
